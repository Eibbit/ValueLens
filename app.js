...
function calculateCropRect() {
    ...
    // Using Math.max(1, ...) to prevent divide by zero
    const scaleX = video.videoWidth / Math.max(1, videoRect.width);
    const scaleY = video.videoHeight / Math.max(1, videoRect.height);
    
    // Calculate relative coordinates
    const relX = boxRect.left - videoRect.left;
    const relY = boxRect.top - videoRect.top;
    
    // Set crop rect with integer bounds
    cropRect = {
        x: Math.max(0, Math.floor(relX * scaleX)),
        y: Math.max(0, Math.floor(relY * scaleY)),
        w: Math.max(10, Math.floor(boxRect.width * scaleX)),
        h: Math.max(10, Math.floor(boxRect.height * scaleY))
    };
    ...
    if (cropRect.w > maxSize || cropRect.h > maxSize) {
        if (cropRect.w > cropRect.h) {
            ...
            canvas.style.height = Math.floor(maxSize * (cropRect.h / cropRect.w)) + 'px';
        } else {
            canvas.style.height = maxSize + 'px';
            canvas.style.width = Math.floor(maxSize * (cropRect.w / cropRect.h)) + 'px';
        }
    } else {
    ...
    console.log("Calculated Crop Rect:", cropRect, "Canvas Size:", canvas.width, "x", canvas.height);
}
